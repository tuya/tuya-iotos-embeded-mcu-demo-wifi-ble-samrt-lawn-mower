...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_DIRECTION_CONTROL, DP_TYPE_ENUM},
	{DPID_BLADE_POSITION_SET, DP_TYPE_ENUM},
	{DPID_BLADE_SPEED_SET, DP_TYPE_ENUM},
	{DPID_LENGTH, DP_TYPE_VALUE},
	{DPID_WIDTH, DP_TYPE_VALUE},
	{DPID_BOW_MODE_SWITCH, DP_TYPE_BOOL},
...
void all_data_update(void)
{
    ...
    mcu_dp_enum_update(DPID_DIRECTION_CONTROL,4); //枚举型数据上报;
	  mcu_dp_enum_update(DPID_BLADE_POSITION_SET,0);
	  mcu_dp_enum_update(DPID_BLADE_SPEED_SET,0);
	  mcu_dp_value_update(DPID_LENGTH,0);
	  mcu_dp_value_update(DPID_WIDTH,0);
	  mcu_dp_bool_update(DPID_BOW_MODE_SWITCH,0);
...
static unsigned char dp_download_direction_control_handle(const unsigned char value[], unsigned short length)
{
    ...
    direction_control = mcu_get_dp_download_enum(value,length);
	  device_info.direction = direction_control;
    
	  move_state.todo_type = manual_control;
    move_state.bow_mode_switch = FALSE;
...
/*****************************************************************************
函数名称 : dp_download_blade_position_handle
功能描述 : 针对DPID_BLADE_POSITION_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_blade_position_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char blade_position;
    
    blade_position = mcu_get_dp_download_enum(value,length);
	
    switch(blade_position) {
			
		case low:
        blade_position_set(low);
		    break;
		
		case medium:
		    blade_position_set(medium);
		    break;
		
		case high:
		    blade_position_set(high);	
		    break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_BLADE_POSITION_SET, blade_position);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_blade_speed_handle
功能描述 : 针对DPID_BLADE_SPEED_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_blade_speed_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char blade_speed;
    
    blade_speed = mcu_get_dp_download_enum(value,length);
	
    device_info.blade_speed = blade_speed;
	
		if(move_state.todo_type != to_stop) {
		    blade_speed_set(blade_speed);
		}
		
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_BLADE_POSITION_SET, blade_speed);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_length_handle
功能描述 : 针对DPID_LENGTH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_length_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned int set_range_length;
    
    set_range_length = mcu_get_dp_download_value(value,length);
	
    move_state.range_length_mm = set_range_length*1000;
    
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_LENGTH, set_range_length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_width_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned int set_range_width;
    
    set_range_width = mcu_get_dp_download_value(value,length);
	
    move_state.range_width_mm = set_range_width*1000;
    
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_LENGTH, set_range_width);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_bow_mode_switch_handle
功能描述 : 针对DPID_BOW_MODE_SWITCH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_bow_mode_switch_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    _Bool bow_mode_switch;
    
    bow_mode_switch = mcu_get_dp_download_bool(value,length);
	
    move_state.bow_mode_switch = bow_mode_switch;
    move_state.todo_type = to_stop;
	
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_LENGTH, bow_mode_switch);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_DIRECTION_CONTROL:
            ...
            break;
        case DPID_BLADE_POSITION_SET:
					  ret = dp_download_blade_position_handle(value,length);
            break;
				case DPID_BLADE_SPEED_SET:
					  ret = dp_download_blade_speed_handle(value,length);
				    break;
				case DPID_LENGTH:
					  ret = dp_download_length_handle(value,length);
				    break;
				case DPID_WIDTH:
					  ret = dp_download_width_handle(value,length);
				case DPID_BOW_MODE_SWITCH:
					  ret = dp_download_bow_mode_switch_handle(value,length);
				    break;
        default:
        break;
    }
